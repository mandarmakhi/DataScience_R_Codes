...
library(readr)
Amtrak <- read.csv("C:/Users/Mandar/Desktop/data/csv files/Amtrak.csv")
...
View(X)#create a martix of 12*12
colnames(X)<-month.abb # Assigning month names 
View(X)# in matrix assigning months name
trakdata<-cbind(Amtrak,X)#add the matrix in the dataset
View(trakdata)
colnames(trakdata)[2]<-"Ridership"
colnames(trakdata)
trakdata["t"]<- 1:120  #add the t in the dataset
View(trakdata)
trakdata["log_rider"]<-log(trakdata["Ridership"])
trakdata["t_square"]<-trakdata["t"]*trakdata["t"] #add the t square data in dataset
#attach(trakdata)
##Data Partition
